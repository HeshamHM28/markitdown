...
def _get_mime_type_prefixes(types: List[DocumentIntelligenceFileType]) -> List[str]:
    ...
    # Use fast dictionary lookups and flat list extension for optimal performance.
    prefixes: List[str] = []
    append = prefixes.extend  # Localize for slight speed gain in large loops.
    for type_ in types:
        if type_ in _MIME_TYPE_PREFIXES:
            append(_MIME_TYPE_PREFIXES[type_])
    return prefixes
