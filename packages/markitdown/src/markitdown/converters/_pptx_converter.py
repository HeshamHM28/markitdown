...
class PptxConverter(DocumentConverter):
    ...
    def _convert_chart_to_markdown(self, chart):
        try:
            ...
            parts = ["\n\n### Chart"]
            if chart.has_title:
                parts.append(f": {chart.chart_title.text_frame.text}")
            parts.append("\n\n")

            plot0 = chart.plots[0]  # Common attribute access
            category_names = [c.label for c in plot0.categories]
            series = chart.series
            series_names = [s.name for s in series]

            # Build table header and separator
            header = "| " + " | ".join(["Category"] + series_names) + " |"
            separator = "|" + "|".join(["---"] * (1 + len(series_names))) + "|"
            ...
            # Handle the specific error for unsupported chart types and any other exceptions
            if isinstance(e, ValueError) and "unsupported plot type" in str(e):
                return "\n\n[unsupported chart]\n\n"
            return "\n\n[unsupported chart]\n\n"
